# Front Desk Orientation Platform - Firm Content Pack

## 1) Orientation Modules

### Module 1: Role of the Front Desk
**Short Explanation**  
The front desk is the first point of contact for every visitor and a control point for the firm. The role sets the tone for the visit and protects service quality across departments.

**Key Rules**
- Welcome every visitor promptly and professionally.
- Treat each interaction as important, even during busy periods.
- Collect and confirm required information before routing anyone.
- Follow protocol consistently to protect service quality and records.
- Stay calm, clear, and respectful in every situation.

### Module 2: Identifying Visitor Purpose
**Short Explanation**  
Every visitor must be correctly identified by purpose before direction is provided. Service lines are Law Firm, Finance, Doctor, and Physiotherapy.

**Key Rules**
- Ask a clear purpose question.
- Confirm the department: Law Firm, Finance, Doctor, or Physiotherapy.
- For Finance visits, do not mention AMSL unless the visitor mentions it first.
- For Finance visits, confirm who the visitor is coming to see and let the visitor direct finance context.
- Do not route based on assumptions or appearance.
- Repeat back the department to confirm understanding.
- Ask a clarifying question if details are unclear.

### Module 3: New vs Existing Client Identification
**Short Explanation**  
After purpose, confirm whether the visitor is new or existing. This affects intake handling and verification steps.

**Key Rules**
- Always ask whether the visitor has been here before.
- Never assume status.
- Verify status before moving to ID and routing.
- Keep a neutral, professional tone.
- If uncertain, classify only after verification.

### Module 4: Identification and Digital Intake Protocol
**Short Explanation**  
Identification collection is a mandatory security and record-keeping requirement. Every visitor must provide a valid government-issued ID for scanning. If a physical ID is unavailable, a Digital Intake Photo must be captured and uploaded to the visitor log with required contact details before routing.

**Key Rules**
- Mandatory verification: Request valid identification from every visitor at arrival.
- Primary method: Prioritize a physical scan or clear photo of government-issued ID.
- Secondary method (no-ID gap): Capture a live digital intake photo and record full legal name plus phone number; record email when available.
- Verification: Confirm identity record matches the visitor and all recorded data is legible.
- Logging requirement: Complete visitor logging in the electronic log when available and in the physical logbook for every visitor.
- Professional firmness: Explain this is required for firm security and insurance compliance.
- Escalation: If a visitor refuses all required identity steps, escalate to the Operations Manager or Managing Partner's office.
- Photo quality procedure: Neutral background, good lighting, unobstructed face, head-and-shoulders framing, and retake if blurred.
- Data pairing control: Confirm the photo is attached to the correct visitor entry before saving.

### Module 5: Routing and Direction
**Short Explanation**  
After purpose, status, and ID are complete, route visitors accurately to the correct team.

**Key Rules**
- Route only after all intake steps are complete.
- For Finance routing, do not introduce AMSL first; route based on visitor-stated purpose and who they are meeting.
- Legal matters -> Legal team.
- Billing/payment matters -> Finance.
- Medical consultation matters -> Doctor.
- Rehabilitation/recovery sessions -> Physiotherapy.

### Module 6: Professional Communication
**Short Explanation**  
Communication must be calm, clear, polite, and confident to build trust and prevent confusion.

**Key Rules**
- Use short, clear sentences.
- Keep tone respectful and neutral.
- Speak confidently without sounding abrupt.
- Listen fully before responding.
- Close each interaction with a clear next step.

---

## 2) Approved Front Desk Scripts

1. **Greeting Script**  
   "Good morning, welcome to the office. My name is [Name], and I'll assist you at the front desk."

2. **Identifying Purpose Script**  
   "May I confirm the purpose of your visit today? Are you here for Law Firm services, Finance, Doctor, or Physiotherapy? If this is a Finance visit, who are you here to see?"

3. **New vs Existing Script**  
   "Thank you. Have you visited our office before, or is this your first appointment with us?"

4. **ID Request Script**  
   "Good morning/afternoon. May I have your ID for our records, please? If you do not have physical ID today, our protocol requires a quick digital intake photo and your contact details for the secure visitor log."

5. **No-ID / Refusal Handling Script**  
   "I understand. In the absence of a physical ID, our protocol requires a quick identification photo and manual contact details. If you prefer, I can explain the requirement clearly. If you still decline, I must escalate to the Operations Manager or Managing Partner's office."

6. **Verification Script**  
   "Thank you. I'm verifying your details now: department [Department], status [New/Existing], identity record complete, and visitor log entries completed in the system and physical logbook."

7. **Routing Script**  
   "Everything is confirmed. Please proceed to the [Department/Area], and the team there will assist you shortly. For Finance visits, I will route you based on the person and purpose you provided."

8. **Closing Script**  
   "Thank you for your cooperation. If you need anything else, please return to the front desk and we'll be glad to help."

---

## 3) Scenario Bank (12)

### Scenario 1: Walk-In Legal Inquiry (New Client)
**Situation:** Visitor needs legal advice and has never visited before.  
**Practice Questions:**  
- What should you ask first to confirm department?  
- How do you confirm new/existing status?  
- How should you request ID professionally?  
- When can you route to legal?

### Scenario 2: Returning Finance Client in a Hurry (Client-Led Routing)
**Situation:** Client says they are here for a finance matter, does not mention AMSL, and asks to skip intake due to delay.  
**Practice Questions:**  
- What must still be verified before routing?  
- How should finance routing be confirmed without introducing AMSL first?  
- How do you respond to pressure to skip protocol?  
- Which logging steps are mandatory before routing?

### Scenario 3: Doctor Appointment with Unclear Purpose
**Situation:** Visitor says "I have an appointment upstairs" without details.  
**Practice Questions:**  
- Which clarifying question identifies department?  
- Why is routing without confirmation risky?  
- Which intake steps must be completed?

### Scenario 4: Physiotherapy Existing Client Without Physical ID
**Situation:** Existing physiotherapy visitor has no physical ID and asks to go directly to treatment.  
**Practice Questions:**  
- What is the required fallback identity method?  
- Which contact details must be recorded before routing?  
- How do you explain the requirement professionally?  
- Can routing happen before identity record completion?

### Scenario 5: Visitor Assumed as Existing by Staff
**Situation:** Colleague suggests routing based on assumption.  
**Practice Questions:**  
- What should be done before routing?  
- Why are assumptions risky?  
- How to confirm status professionally?

### Scenario 6: No-ID Visitor in Finance Intake
**Situation:** New finance visitor has no physical ID, agrees to digital intake, but the first image is blurred/backlit and the electronic log is temporarily unavailable.  
**Practice Questions:**  
- What quality checks must be completed before acceptance?  
- What should be done if the image is blurred or obscured?  
- What data pairing step is required before saving?
- What physical logbook action is still required when the electronic log is unavailable?

### Scenario 7: Family Member Checking In for Doctor Visit
**Situation:** Family member answers on behalf of patient.  
**Practice Questions:**  
- How to verify who the client is?  
- Whose status should be confirmed?  
- What to do about ID collection?

### Scenario 8: Cross-Department Confusion (Legal vs Finance)
**Situation:** Visitor requests payment and legal file help in same visit.  
**Practice Questions:**  
- How to identify primary department first?  
- What must be verified before routing?  
- How to communicate next steps clearly?

### Scenario 9: Existing Physiotherapy Client with Name Mismatch
**Situation:** Visitor claims existing status but provided name differs from record.  
**Practice Questions:**  
- Which verification step is priority?  
- How does ID help resolve mismatch?  
- Should routing wait until clarified?

### Scenario 10: New Doctor Client Without ID and Limited Time
**Situation:** New doctor visitor has no physical ID and asks to skip intake due to time pressure.  
**Practice Questions:**  
- Which identity steps are non-negotiable?  
- What is the correct digital intake fallback script?  
- What routing decision applies if identity record is incomplete?

### Scenario 11: Logging Omission During Busy Front Desk Period
**Situation:** Staff completes electronic intake during a rush but forgets to enter two visitors in the physical logbook.  
**Practice Questions:**  
- What compliance issue occurred?  
- What corrective action is required immediately?  
- Can visitors be treated as fully logged with electronic-only entries?

### Scenario 12: Uncertain Walk-In with No ID and Incomplete Details
**Situation:** Visitor is unsure of department and has no physical ID, and is hesitant about being photographed.  
**Practice Questions:**  
- Best first question to narrow purpose?  
- How to determine new/existing status?  
- How should digital intake requirement be explained professionally?  
- What escalation path applies if visitor refuses both ID and digital intake?  
- Can routing happen before identity record and purpose are confirmed?

---

## 4) Assessment Question Bank (50 Questions)

**Q01 (Greeting & Professionalism)**  
What is the best opening line for a first-time visitor?  
A. Please wait there.  
B. Next.  
C. Good morning, welcome to the office. How may I assist you today?  
D. Do you have all your paperwork?  
E. Are you late for your appointment?  
**Correct: C**

**Q02 (Client Identification)**  
Which question best identifies visitor purpose?  
A. Are you here for someone important?  
B. Which department are you visiting today: Law Firm, Finance, Doctor, or Physiotherapy?  
C. Can you go upstairs now?  
D. Do you know where to go?  
E. Are you paying today?  
**Correct: B**

**Q03 (New vs Existing Handling)**  
What is the correct approach to new vs existing status?  
A. Assume existing if face looks familiar.  
B. Ask only new visitors.  
C. Always ask whether the person has visited before and verify.  
D. Skip this if busy.  
E. Let client decide without verification.  
**Correct: C**

**Q04 (ID Protocol)**  
Which statement reflects office ID protocol?  
A. ID optional for existing clients.  
B. ID only for legal clients.  
C. ID must be collected as copy or photo for records.  
D. ID can be skipped if visitor is in a hurry.  
E. ID only when supervisor is present.  
**Correct: C**

**Q05 (Routing)**  
A visitor is here for billing questions. Where should they be routed?  
A. Legal  
B. Finance  
C. Doctor  
D. Physiotherapy  
E. Any office  
**Correct: B**

**Q06 (Scenario Judgment)**  
Visitor says they are seeing someone upstairs but gives no department details. First step?  
A. Send upstairs quickly.  
B. Ask clarifying question to identify exact department.  
C. Call security.  
D. Assume legal.  
E. Ask payment details first.  
**Correct: B**

**Q07 (Greeting & Professionalism)**  
Which tone is most appropriate?  
A. Casual/joking  
B. Abrupt  
C. Calm, respectful, clear  
D. Overly personal  
E. Silent  
**Correct: C**

**Q08 (Client Identification)**  
Why identify purpose before routing?  
A. Reduce conversation only  
B. Avoid status confirmation  
C. Ensure accurate direction to correct service line  
D. Avoid ID  
E. Let visitors choose randomly  
**Correct: C**

**Q09 (New vs Existing Handling)**  
Client says they visited before. What next?  
A. Skip verification  
B. Verify details before proceeding  
C. Mark as new  
D. Route immediately  
E. Ask staff to guess  
**Correct: B**

**Q10 (ID Protocol)**  
Best way to request ID?  
A. Give me your ID now.  
B. You can skip ID if quick.  
C. As part of standard intake, we need copy/photo of ID for records.  
D. Not sure ID is required today.  
E. Show ID only if you want.  
**Correct: C**

**Q11 (Routing)**  
Client here for physical recovery sessions. Department?  
A. Law Firm  
B. Finance  
C. Doctor  
D. Physiotherapy  
E. Reception only  
**Correct: D**

**Q12 (Scenario Judgment)**  
Visitor has no physical ID and says they have one quick question. Best response?  
A. Allow entry without ID  
B. End conversation  
C. Explain identity record is mandatory, complete digital intake photo plus required contact details, and escalate only if refusal continues  
D. Return another day without documentation  
E. Ask another client  
**Correct: C**

**Q13 (Greeting & Professionalism)**  
What should be avoided in front desk conversations?  
A. Clear instructions  
B. Polite confirmation  
C. Assumptive language and dismissive tone  
D. Professional greeting  
E. Calm pace  
**Correct: C**

**Q14 (Client Identification)**  
Which is a correct purpose category in this office?  
A. Human Resources  
B. Marketing  
C. Finance  
D. IT Support  
E. Sales  
**Correct: C**

**Q15 (New vs Existing Handling)**  
When should status be determined?  
A. After routing  
B. Before verification and routing  
C. Only finance  
D. Only new legal  
E. Not needed  
**Correct: B**

**Q16 (ID Protocol)**  
What must be verified after collecting ID?  
A. Nothing else  
B. Image is clear and details match visitor  
C. First name only  
D. Visitor smile  
E. Appointment time only  
**Correct: B**

**Q17 (Routing)**  
Legal consultation visitors should go to:  
A. Doctor  
B. Finance  
C. Physiotherapy  
D. Legal team  
E. Storage area  
**Correct: D**

**Q18 (Scenario Judgment)**  
Existing client says "you know me, skip questions." What should you do?  
A. Skip questions  
B. Explain verification is standard and continue protocol  
C. Ask them to leave  
D. Route then verify  
E. Ask another visitor to confirm  
**Correct: B**

**Q19 (Greeting & Professionalism)**  
Best closing after successful intake?  
A. You're done. Next.  
B. Go wherever you think is right.  
C. Thank you. Please proceed to assigned department; we are here if needed.  
D. Don't come back without documents.  
E. No closing needed.  
**Correct: C**

**Q20 (Client Identification)**  
If visitor gives vague purpose, you should:  
A. Guess department  
B. Ask clarifying question  
C. Default finance  
D. Skip steps  
E. Ask unrelated personal questions  
**Correct: B**

**Q21 (New vs Existing Handling)**  
Why is "never assume" important?  
A. Speeds service  
B. Reduces documentation  
C. Prevents routing and record errors  
D. Required in busy offices  
E. Improves privacy  
**Correct: C**

**Q22 (ID Protocol)**  
Which meets ID protocol?  
A. No ID for returning clients  
B. ID only for doctor visits  
C. Physical ID is verified, or digital intake photo plus full legal name and phone number is captured and verified  
D. Verbal confirmation only  
E. Any document without verification  
**Correct: C**

**Q23 (Routing)**  
Visitor says they are here for a finance payment matter but does not mention AMSL. Best front desk approach?  
A. Ask if they want AMSL before asking anything else  
B. Confirm Finance visit, ask who they are here to see, and route based on visitor-stated purpose  
C. Route to legal by default  
D. Tell them to decide later upstairs  
E. Skip intake if they appear confident  
**Correct: B**

**Q24 (Scenario Judgment)**  
Visitor walks past desk toward offices. What should you do?  
A. Ignore  
B. Politely stop and complete intake first  
C. Shout  
D. Let proceed and record later  
E. Ask another client  
**Correct: B**

**Q25 (ID Protocol)**  
Which statement correctly reflects the firm's visitor logging requirement?  
A. Electronic log is enough if the day is busy  
B. Physical logbook is optional when the system is working  
C. Every visitor must be logged in the physical logbook, and electronic logging is also completed when available  
D. Logging is only required for new visitors  
E. Logging can be completed after routing  
**Correct: C**

**Q26 (Client Identification)**  
Which list includes all valid service lines?  
A. Legal, Marketing, IT, Doctor  
B. Law Firm, Finance, Doctor, Physiotherapy  
C. Finance, Sales, HR, Physiotherapy  
D. Doctor, IT, Finance, Security  
E. Law Firm, Accounting, Surgery, Therapy  
**Correct: B**

**Q27 (New vs Existing Handling)**  
Colleague says "I know this person." Best action?  
A. Skip checks  
B. Verify status directly with visitor  
C. Send to any department  
D. Ask visitor to come later  
E. Ignore visitor  
**Correct: B**

**Q28 (ID Protocol)**  
Why communicate ID requirement clearly?  
A. So staff can choose who to process  
B. Because records protocol applies to all visitors  
C. Optional but preferred  
D. Only supervisors asked once  
E. Replaces all verification  
**Correct: B**

**Q29 (Routing)**  
Visitor has scheduled medical consultation. Route to:  
A. Law Firm  
B. Finance  
C. Doctor  
D. Physiotherapy  
E. Parking office  
**Correct: C**

**Q30 (Scenario Judgment)**  
Physiotherapy client is late and has no physical ID. Correct response?  
A. Allow exception automatically  
B. Explain identity record is required before routing and offer quick digital intake photo plus required contact details  
C. Mark absent  
D. Return next week  
E. Skip checks for this department  
**Correct: B**

**Q31 (Scenario Judgment / Logic)**  
Correct intake order?  
A. Identify Purpose -> New/Existing -> ID -> Route  
B. New/Existing -> Purpose -> Route -> ID  
C. Route -> Purpose -> New/Existing -> ID  
D. ID -> Purpose -> New/Existing -> Route  
E. Purpose -> ID -> New/Existing -> Route  
**Correct: A**

**Q32 (Client Identification)**  
For Finance visits, what is the correct way to handle AMSL in conversation?  
A. Always mention AMSL first so clients can choose  
B. Do not mention AMSL unless the visitor mentions it; confirm Finance and ask who they are here to see  
C. Avoid finance clarification to prevent delays  
D. Route all finance visitors to AMSL unless they object  
E. Ask another client to clarify the visitor's route  
**Correct: B**

**Q33 (Client Identification)**  
Best purpose confirmation phrase?  
A. You're probably legal, correct?  
B. Let me confirm: you are here for [Department] service today.  
C. Decide later.  
D. I'll choose for you.  
E. Not necessary.  
**Correct: B**

**Q34 (New vs Existing Handling)**  
If visitor is unsure whether they visited before:  
A. Skip question  
B. Assume new and route  
C. Verify carefully before classification  
D. Return with proof only  
E. Classify randomly  
**Correct: C**

**Q35 (ID Protocol)**  
Best response when asked why ID or digital intake photo is needed?  
A. Because I said so.  
B. No reason, just policy.  
C. Mandatory requirement for firm security, insurance compliance, and record verification.  
D. Only today.  
E. Only new clients.  
**Correct: C**

**Q36 (Routing)**  
Visitor comes for post-injury rehabilitation. Route to:  
A. Law Firm  
B. Finance  
C. Doctor  
D. Physiotherapy  
E. Lobby seating only  
**Correct: D**

**Q37 (Scenario Judgment)**  
Visitor is here for Finance but is unsure which finance office name applies. Best first step?  
A. Introduce AMSL immediately and ask if they were approved  
B. Confirm who they are here to see and their stated finance purpose, then route without introducing internal recommendation logic  
C. Route to legal temporarily  
D. Skip intake and let them ask upstairs  
E. Reject visit unless they know the office name  
**Correct: B**

**Q38 (Greeting & Professionalism)**  
Which shows professional consistency?  
A. Ask full questions only when supervisors watch  
B. Follow same protocol for every visitor  
C. Skip steps for familiar faces  
D. Avoid explanations  
E. Let visitors create process  
**Correct: B**

**Q39 (Client Identification)**  
Risk of routing before purpose confirmation?  
A. No risk  
B. Delays, confusion, service errors  
C. Improves efficiency  
D. Removes need for ID protocol  
E. Affects only new clients  
**Correct: B**

**Q40 (Scenario Judgment / Logic)**  
Existing finance visitor has no physical ID and refuses digital intake photo. Correct next action?  
A. Route immediately  
B. Restate mandatory identity requirements professionally and escalate to Operations Manager or Managing Partner's office if refusal continues  
C. Reclassify as legal  
D. Skip ID and note refusal  
E. End service without explanation  
**Correct: B**

**Q41 (New vs Existing Handling)**  
Which statement fits protocol?  
A. Existing clients do not need confirmation  
B. New/existing should be confirmed for every visitor  
C. Status only for legal matters  
D. Status can be guessed  
E. Optional when busy  
**Correct: B**

**Q42 (ID Protocol)**  
What qualifies as acceptable ID handling?  
A. Quick glance without record  
B. Verbal ID number  
C. Collect physical ID record or digital intake photo fallback with full legal name and phone number, then verify match and legibility  
D. Record initials only  
E. Skip if visitor appears trustworthy  
**Correct: C**

**Q43 (Routing)**  
If intake is incomplete, routing should:  
A. Proceed anyway  
B. Pause until required steps are complete  
C. Default to legal  
D. Delegate to waiting visitors  
E. Be skipped entirely  
**Correct: B**

**Q44 (Scenario Judgment)**  
Visitor insists they were told "no ID or photo needed today." What should you do?  
A. Accept claim and skip policy  
B. Calmly restate mandatory identity protocol, continue required steps, and escalate if refusal continues  
C. Argue until they agree  
D. Ignore and route  
E. Send home without explanation  
**Correct: B**

**Q45 (ID Protocol)**  
If electronic logging is temporarily unavailable, what should front desk do?  
A. Pause all check-ins until the system returns  
B. Skip logging and continue routing  
C. Complete physical logbook entries immediately and complete electronic entries once available, without skipping required intake controls  
D. Log only new visitors  
E. Let visitors self-log later  
**Correct: C**

**Q46 (Scenario Judgment / Logic)**  
New doctor visitor with unclear details. Best sequence?  
A. Route -> ask new -> request ID -> confirm purpose  
B. Confirm purpose -> ask new/existing -> request and verify ID -> route  
C. Request ID -> route -> confirm purpose -> ask status  
D. Ask status -> route -> confirm purpose -> request ID  
E. Route and complete later  
**Correct: B**

**Q47 (Client Identification)**  
Visitor says they are here for recovery sessions. Most likely department?  
A. Law Firm  
B. Finance  
C. Doctor  
D. Physiotherapy  
E. Records office  
**Correct: D**

**Q48 (New vs Existing Handling)**  
Best professional status question?  
A. You're not new, right?  
B. Have you visited our office before, or is this your first time?  
C. You look familiar, so I'll mark existing.  
D. I don't need to ask this today.  
E. Tell me quickly: new or old?  
**Correct: B**

**Q49 (ID Protocol)**  
If digital intake photo is blurry or backlit, correct action is to:  
A. Accept to save time  
B. Proceed without record  
C. Retake under proper lighting and verify attachment to the correct visitor record before routing  
D. Ignore and mark complete  
E. Route first, verify later  
**Correct: C**

**Q50 (Routing)**  
All intake steps are complete for existing legal client. What next?  
A. Repeat full intake  
B. Hold at desk without reason  
C. Route clearly to legal team  
D. Send to finance first  
E. Ask client to self-navigate  
**Correct: C**

---

## 5) Core Intake Rule (For Supervisor Emphasis)

**Required order**
1. Identify Purpose  
2. Confirm New/Existing  
3. Collect and Verify Identity Record  
4. Route Visitor

**Hard rule**
- Routing must not occur before the identity record is completed and verified (physical ID or approved digital intake fallback).

## 6) Operational Controls (Supervisor Emphasis)

- Finance conversation boundary: Front desk does not introduce AMSL first. For Finance visits, confirm who the visitor is here to see and route based on visitor-stated purpose.
- Logging requirement: Physical logbook entry is required for every visitor. Electronic logging is completed when available and does not replace the physical logbook requirement.
